...
export const useIngredients = (props) => {
    ...
    const currentDictionaryName = computed(() => {
        return getCurrentDictionaryName('ingredients', t);
    });

    const isCategorySelected = computed(() => {
        return formData.value.category && formData.value.category !== '';
    });
    ...
    const loadItemsData = async () => {
        loading.value = true;
        try {
            const data = await loadItems('ingredients');
    ...
    const saveItemData = async () => {
        const success = await saveItem('ingredients', editingItem.value, formData.value);
        if (success) {
            await loadItemsData();
            closeModal();
        }
    ...
    const deleteItemData = async (item) => {
        if (!confirm(t('ingredients.delete_confirm', { name: item.name }))) {
            return;
        }

        const success = await deleteItem('ingredients', item.id);
        if (success) {
            await loadItemsData();
        }
    };
    ...
    const getDisplayFieldsData = () => {
        return getDisplayFields('ingredients', fields.value);
    };

    const getFieldLabel = (fieldName) => {
        return t(`ingredients.fields.${fieldName}`);
    };
